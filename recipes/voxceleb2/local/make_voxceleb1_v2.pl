...
if ($dataset eq "test"){
  ...
  if (system("mkdir -p $out_dir/trials") != 0) {
    die "Error making directory $out_dir/trials";
  }
  open(TRIAL_OUT, ">", "$out_dir/trials/voxceleb1-O") or die "Could not open the output file $out_test_dir/trials/voxceleb1-O";
