...
# sys.path.insert(0, "/data/lijianchen/workspace/sre/speaker-net")
from speakernet.utils.rich_utils import track
from speakernet.preprocessing.signal_processing import reverberate
from speakernet.utils.utils import load_data_csv, is_primary_process
...
class NoiseData:
    def __init__(self, csv_file, padding=0, pre_load=False):
        ...
        if self.pre_load:
            self.id2waveforms = {}
            for id in track(self.data_ids, description="Loading the noise data..."):
                self.id2waveforms[id] = torchaudio.load(self.data[id]["wav"])[0]
    ...
    def load(self, id, max_len=None):
        ...
        if max_len is not None:
            if audio_len >= max_len:
                ...
                if self.pre_load:
                    waveforms = self.id2waveforms[id][:, start : start + max_len]
                else:
                ...
                if self.pre_load:
                    waveforms = self.id2waveforms[id]
                else:
...
class AddNoise(torch.nn.Module):
    ...
    pre_load: bool
        Pre-load all noise audios into memory. This is useful if the noise
        audios are too long.
    """
    ...
    def __init__(
        ...
        self.noises_data = NoiseData(csv_file, padding=pad_noise, pre_load=pre_load)
...
class AddReverb(torch.nn.Module):
    ...
    def __init__(self, csv_file, reverb_prob=1.0, rir_scale_factor=1.0, pre_load=False):
        super().__init__()
        ...
        # RIR waveforms
        assert csv_file is not None
        self.rir_data = NoiseData(csv_file, pre_load=pre_load)
...
class SpeedPerturb(torch.nn.Module):
    ...
    def forward(self, waveform: torch.Tensor, speed_factor: Optional[float] = None):
        """
        Arguments
        ---------
        waveforms : tensor
...
class AugList:
    def __init__(self, augments: List[Union[EnvCorrupt, SpeedPerturb, TempoPerturb]]):
...
class SpeechAug(torch.nn.Module):
    ...
    def load_aug_class(self, aug_classes, mod):
        ...
        for aug_class in aug_classes:
            ...
            if "aug_classes" in aug_class:
                ...
                if mod == "random":
                    # Append the weight of the nested augment to the list of returned weights, then pop it
    ...
    def forward(self, waveforms: torch.Tensor, speed_factor: Optional[float] = None):
    ...
    def _random_forward(
        ...
        if aug_idx == 0:
            ...
            if type(augment) == list:
                ...
                if type(augment) == SpeedPerturb:
                    assert label_multiplier == 0, "SpeedPerturb can only be used once."
                    waveforms, label_multiplier = augment(waveforms, speed_factor)
                else:
...
if __name__ == "__main__":
    ...
    with open(
        "/data/lijianchen/workspace/sre/speaker-net/recipes/voxceleb2/hparams/speech_aug_random_tmp.yaml",
        "r",
    ) as fin:
