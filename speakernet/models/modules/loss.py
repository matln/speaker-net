...
# sys.path.insert(0, "./")
from speakernet.models.modules.framework import SpeakerLoss
...
class SoftmaxLossFunc(SpeakerLoss):
    ...
    def init(self, t=1, reduction="mean", label_smoothing=0.0, soft_target=False):
        self.t = t  # temperature
...
class MarginSoftmaxLoss(SpeakerLoss):
    """Margin softmax loss.
    Support AM, AAM, subcenter, inter-topk.
    ...
    def init(
        ...
        self.t = t  # temperature
        self.K = K  # subcenter
        self.topk = topk  # number of hard samples
        self.topk_m = topk_m  # margin penalty of hard samples
        ...
        self.cos_topk_m = math.cos(self.topk_m)
        self.sin_topk_m = math.sin(self.topk_m)
    ...
    def forward(self, inputs, targets):
        ...
        if self.method == "am":
            ...
            if self.topk_m > 0:
                ...
                # exclude j = y_i
                assert torch.max(cosine) <= 1.0
                topk_cosine, topk_targets = torch.topk(cosine - 2 * one_hot, self.topk, dim=-1)
                topk_cosine_phi = topk_cosine + self.topk_m
        elif self.method == "aam":
            cosine = cosine.float()
            cosine_target = cosine_target.float()
            # phi = torch.cos(torch.acos(cosine_target) + self.m)

            # cosine(theta+m) = cosine * cos_m - sin_theta * sin_m
            ...
            if self.topk_m > 0:
                ...
                topk_cosine_phi = self.cos_topk_m * topk_cosine + self.sin_topk_m * torch.sqrt(
                    (1 - torch.pow(topk_cosine, 2)).clamp(0, 1)
                )
        else:
            raise ValueError(
        ...
        if self.topk_m > 0:
            topk_cosine = (
                1 / (1 + self.lambda_factor) * topk_cosine_phi
                + self.lambda_factor / (1 + self.lambda_factor) * topk_cosine
            )
            cosine = cosine.scatter(1, topk_targets, topk_cosine)
...
class MarginSoftmaxLossFunc(SpeakerLoss):
    ...
    def forward(self, cosine, targets):
        ...
        if self.method == "am":
            ...
            # cos(\theta + m)在\theta(维度为(batch_size, num_classes))接近\pi
            # 时就不是关于\theta的单调递减函数了，
            # 但\phi中其实只有对应于真实标签的那些列会被用到，一般情况下\theta
            # 是很小的。在训练初期也有可能
            # 出现\theta_{y_i} + m大于\pi。下面easy_margin就是相当于训练初期用
            # original softmax，随着训练进行
            # \theta_{y_i}会逐渐减小，就会由\cos(\theta)变为\cos(\theta + m)。
            # 至于不是easy_margin无非就是将\cos(\theta + m)变为[0, \pi]的单调递减函数。
            if self.easy_margin:
                phi = torch.where(cosine_target > 0, phi, cosine_target)
            else:
                # phi = torch.where(cosine_target > self.th, phi, cosine_target - self.mm)
