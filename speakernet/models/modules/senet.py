...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.full_pre_activation:
            self._full_pre_activation(
                inplanes, planes, Conv, stride, norm_layer, norm_layer_params, se_ratio
            )
        else:
    ...
    def _full_pre_activation(
        self, inplanes, planes, Conv, stride, norm_layer, norm_layer_params, se_ratio
    ):
...
class SEResNet(nn.Module):
    ...
    def __init__(
        ...
        if not head_conv and self.in_planes != head_inplanes:
            raise ValueError(
                "The inplanes is not equal to resnet first block"
        ...
        self.layer2 = self._make_layer(
            used_block, planes[1], layers[1], stride=2, dilate=replace_stride_with_dilation[0],
        )
        self.layer3 = self._make_layer(
            used_block, planes[2], layers[2], stride=2, dilate=replace_stride_with_dilation[1],
        )
        self.layer4 = self._make_layer(
            used_block, planes[3], layers[3], stride=2, dilate=replace_stride_with_dilation[2],
        )
