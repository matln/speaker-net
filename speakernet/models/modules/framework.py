...
class SpeakerNet(torch.nn.Module):
    ...
    init():
        just like pytorch needed. Note there is 'init' rather than '__init__'.
    forward(*inputs):
        just like pytorch needed.
    get_loss(*inputs, targets):
        to support fetching the final loss from multi-loss.
    get_posterior():
        to compute accuracy.
    extract_embedding(inputs) :
        needed if use pipline/onestep/extract_embeddings.py.
    """
        ...
        if self.use_step:
            if self.step_params["m"]:
                ...
                if len(increase_start_epoch) == 2:
                    increase_start_position = int(increase_start_epoch[0]) * epoch_batchs + int(
