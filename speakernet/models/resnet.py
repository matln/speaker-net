...
class Encoder(SpeakerNet):
    ...
    def init(
        ...
        # 将 channel 和 feat_dim 乘在了一起，因为statisticpoolinglayer只接受3维的tensor
        # feat_dim = ceil(inputs_dim / self.resnet.get_downsample_multiple())
        resnet_output_dim = (
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.LongTensor) -> torch.Tensor:
        """
        inputs: a 3-dimensional tensor [batch, frames-dim-index, frames-index]
        """
        x = inputs
...
if __name__ == "__main__":
    encoder = Encoder(num_targets=5994 * 3, pooling="multi-head")
