...
from speakernet.utils.utils import is_primary_process

logger = logging.getLogger('fit_progressbar')

CKPT_PREFIX = "epoch"
METAFNAME = "CKPT.yaml"  # Important that this is not .ckpt
...
DEFAULT_LOAD_HOOKS = {
    ...
    torch.cuda.amp.GradScaler: torch_recovery
}
DEFAULT_SAVE_HOOKS = {
    torch.nn.Module: torch_save,
    torch.optim.Optimizer: torch_save,
    torch.cuda.amp.GradScaler: torch_save
}
...
def mark_as_loader(method):
    ...
    try:
        ...
        raise TypeError(MSG)
    method._speakernet_loader = True
    return method
...
def mark_as_transfer(method):
    ...
    try:
        ...
        raise TypeError(MSG)
    method._speakernet_transfer = True
    return method
...
class Checkpointer:
    ...
    def save_checkpoint(
        self, meta={}, end_of_epoch=True, epoch=None, name=None, verbosity=logging.INFO
    ):
        """Saves a checkpoint.
        ...
        """
        assert epoch is not None or name is not None
        if name is None:
            ckpt_dir = self._new_checkpoint_dirpath(epoch)
        else:
            ckpt_dir = self._custom_checkpoint_dirpath(name)
        shutil.rmtree(ckpt_dir, ignore_errors=True)  # os.makedirs may raise FileExistsError.
    ...
    def save_and_keep_only(
        ...
        meta : mapping, optional
            A mapping which is added to the meta file in the checkpoint. The
            key "unixtime" and "end_of_epoch" are included by default.
        end_of_epoch : bool, optional
            Whether the checkpoint is at the end of an epoch. True by default.
            May affect loading.
        epoch : str
        ...
        name : str, optional
            Specify a custom name for your checkpoint.
            The name will still have a prefix added. If no name is given,
            a name is created from the above epoch.
        ...
        rencent_num_to_keep : int, optional
            Number of recent checkpoints to keep. Defaults to 1. This deletes all
            checkpoints remaining after filtering. Must be >=0.
        ...
        self.save_checkpoint(
            meta=meta, end_of_epoch=end_of_epoch, epoch=epoch, name=name, verbosity=verbosity
        )

        if keep_recent:
            importance_keys.append(ckpt_recency)
            assert ckpt_predicate is not None
    ...
    def delete_checkpoints(
        ...
        *,
        ...
        num_to_keep : int, optional
            ...
            Defaults to 1. You choose to keep 0. This deletes all
            checkpoints remaining after filtering. Must be >=0
        rencent_num_to_keep : int, optional
            Number of recent checkpoints to keep. Defaults to 1. This deletes all
            checkpoints remaining after filtering. Must be >=0.
        ...
        if recent_num_to_keep < 0:
            raise ValueError("Number of recent checkpoints to keep must be positive.")
        ...
        for key_kwargs in keys:
            ...
            if (
                "importance_key" in key_kwargs
                and key_kwargs["importance_key"] is ckpt_recency
            ):
                protected_checkpoints.update(
                    self.find_checkpoints(
                        max_num_checkpoints=recent_num_to_keep, **key_kwargs
                    )
                )
            else:
                protected_checkpoints.update(
    ...
    def _list_checkpoint_dirs(self):
        ...
        if pathlib.Path.exists(self.checkpoints_dir):
            return [
                x
