...
## Wrapper ✿
def get_optimizer(params, opt_params: dict = {}):
    ...
    used_params = utils.assign_params_dict(default_params, opt_params)
    ...
    if gc:
        ...
        if name not in default_support_gc_list:
            raise TypeError(
                "Optimizer {} does not support gradient centralization (GC) now.".format(
                    name
                )
            )
    ...
    if name == "sgd":
        ...
        base_optimizer = SGDW(
            params, lr=learn_rate, momentum=beta1, weight_decay=weight_decay
        )
    elif name == "sgdW_original":
        base_optimizer = SGDW_O(
            params, lr=learn_rate, momentum=beta1, weight_decay=weight_decay
        )
    elif name == "adam":
    ...
    elif name == "adamW_original":
        base_optimizer = AdamW_O(
            params,
            lr=learn_rate,
            betas=(beta1, beta2),
            weight_decay=weight_decay,
        ...
        base_optimizer = RAdam(
            params, lr=learn_rate, betas=(beta1, beta2), weight_decay=weight_decay
        )
    elif name == "ralamb":
    ...
    if used_params["lookahead.alpha"] > 0:
        logger.info(
            "Use lookahead optimizer with alpha={} and k={}".format(
...
class SGDW(Optimizer):
    ...
    def __init__(
        self, params, lr=0.1, momentum=0, dampening=0, weight_decay=0, nesterov=False
    ):
        if lr < 0.0:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if group["weight_decay"] != 0:
                    # 去掉原论文中引入的ScheduleMultiplier eta，直接
                    # 在 lr 上进行 cosine 模拟退火，并在一开始放大 weight decay 的大小（除以 init lr）
                    # 这样就可以和 SGDR 共用 一个函数（lr_scheduler.py）
...
class SGDW_O(Optimizer):
    ...
    .. note::
        The implementation of SGD with Momentum/Nesterov is consistent with
        Sutskever et. al. and implementations in some other frameworks.
    ...
    def __init__(
        ...
        if nesterov and (momentum <= 0 or dampening != 0):
            raise ValueError("Nesterov momentum requires a momentum and zero dampening")
        super(SGDW_O, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(SGDW_O, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault("nesterov", False)
...
class AdamW_O(Optimizer):
    ...
    def __init__(
        ...
        super(AdamW_O, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(AdamW_O, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault("amsgrad", False)
            ...
            for p in group["params"]:
                ...
                if N_sma > self.N_sma_threshhold:
                    ...
                    p_data_fp32.addcdiv_(
                        exp_avg, denom, value=-radam_step * trust_ratio
                    )
                else:
...
class AdaMod(Optimizer):
    ...
    def __init__(
        self, params, lr=1e-3, betas=(0.9, 0.999), beta3=0.999, eps=1e-8, weight_decay=0
    ):
        if not 0.0 <= lr:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if not 0.0 <= eps:
