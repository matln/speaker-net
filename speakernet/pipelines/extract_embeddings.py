...
parser.add_argument("--deploy-embedding-model", type=str, default="encoder_deploy.ckpt",
                    help="Model params file after performing re-parameterization. Especially for RepVGG.")
parser.add_argument("--re-param", type=str, action=StrToBoolAction, default=False,
                    choices=["true", "false"], help="Whether re-parameterization is needed. For RepVGG")
...
# >>> Set seed
utils.set_seed(
    hparams["seed"], deterministic=hparams["deterministic"] if "deterministic" in hparams else True
)

# Set checkpoints dir
checkpoints_dir = "checkpoints"
if "rank0" in os.listdir(f"{model_dir}/checkpoints"):
    checkpoints_dir = "checkpoints/rank0"
        ...
        while True:
            if args.extract_epochs == "":
                while True:
                    ...
                    extract_embs = []
                    for checkpoint in os.listdir(f"{model_dir}/{checkpoints_dir}"):
                        with open(f"{model_dir}/{checkpoints_dir}/{checkpoint}/CKPT.yaml") as fi:
                            ...
                            for position in extract_positions:
                                if f"{position}_epoch_{epoch}" not in extracted_embs and int(
                                    epoch.split(".")[0]
                                ) >= int(args.lower_epoch.split(".")[0]):
                                    if (
                                        int(epoch.split(".")[0])
                                        > int(args.lower_epoch.split(".")[0])
                                        or "." not in args.lower_epoch
                                        or "." in epoch
                                        and "." in args.lower_epoch
                                        and int(epoch.split(".")[1])
                                        >= int(args.lower_epoch.split(".")[1])
                                    ):
            ...
            for position, epoch in extract_embs:
                extract_config = "{0}_extract.config".format(position)
                if args.re_param:
                    extract_config = "re_{0}_extract.config".format(position)
                ...
                if not os.path.exists(extract_config_path):
                    ...
                    # The position of extracting embeddings depends on the 'extracted_embedding' parameter in the hparams file.
                    ...
                    if "extracted_embedding" in hparams["encoder_params"]:
                        encoder_creation = encoder_creation.replace(
                            hparams["encoder_params"]["extracted_embedding"],
                            position,
                        )

                    # For RepVGG
                    if args.re_param:
                        encoder_creation = encoder_creation.replace("deploy=False", "deploy=True")
                ...
                encoder_params = f"{model_dir}/{checkpoints_dir}/epoch+{epoch}/{args.embedding_model}"
                ...
                if args.re_param:
                    # TODO
                    deploy_encoder_params = f"{model_dir}/checkpoints/epoch+{epoch}/{args.deploy_embedding_model}"

                    if not os.path.isfile(deploy_encoder_params):
                        encoder_blueprint, encoder_creation = utils.read_nnet_config(
                            "{0}/config/nnet.config".format(model_dir)
                        )
                        encoder = utils.import_module(encoder_blueprint, encoder_creation)
                        state_dict = {k.replace("module.", ""): v for k, v in torch.load(encoder_params, map_location="cpu").items()}
                        encoder.load_state_dict(state_dict, strict=False)
                        _ = encoder.convert_model(encoder, deploy_encoder_params, do_copy=False)
                        del encoder, state_dict, encoder_blueprint, encoder_creation

                    encoder_params = deploy_encoder_params
