...
def extract(
    wav_paths, gpu_id, job_id, model_params_path, nnet_config, out_dir, replacements, counter, lock
):
    try:
        model_blueprint, model_creation = utils.read_nnet_config(nnet_config, log=False)
        if "features='kaldi_fbank'" in model_creation:
            model_creation = re.sub(
                r"'device': 'cuda:\d+'", f"'device': 'cuda:{gpu_id}'", model_creation
            )
        model = utils.import_module(model_blueprint, model_creation)
        state_dict = {
            k.replace("module.", ""): v
            for k, v in torch.load(model_params_path, map_location="cpu").items()
        }
        ...
        vectors_wspecifier = (
            f"ark,scp:{out_dir}/xvector.{job_id}.ark,{out_dir}/xvector.{job_id}.scp"
        )
                ...
                if seg[1] == "sox":
                    ...
                    for wav in wavs:
                        if replacements[0] == "none":
                            ...
                            _sig, _ = torchaudio.load(wav.replace(replacements[0], replacements[1]))
                        sigs.append(_sig)
                    ...
                    if replacements[0] == "none":
                        ...
                        sig, fs = torchaudio.load(wav.replace(replacements[0], replacements[1]))
