...
class KaldiDataset():
    ...
    == Mapping files ==
        ...
        self.utt2domain: dict{str:str}
        self.domain2utt: dict{str:list[str]}
    ...
    def __init__(self, data_dir: str = "",
        ...
        self.domain_first_files = [
            ("domain2utt", "domain2utt", "str", True)]
    ...
    @classmethod
    def load_data_dir(cls, data_dir: str, expected_files: list = ["utt2spk", "spk2utt", "feats.scp", "utt2num_frames"], wav_scp_vector=False):
        return cls(data_dir, expected_files, wav_scp_vector=wav_scp_vector)
    ...
    def load_data_(self):
        ...
        if self.more:
            logger.info("Loading mapping files form {0} as more as possible with more=True".format(
                self.data_dir))
        else:
            logger.info("Loading mapping files form {0} w.r.t expected files {1}".format(
                self.data_dir, self.expected_files))

        for attr, file_name, value_type, vector in self.utt_first_files + self.spk_first_files + self.domain_first_files:
            file_path = "{0}/{1}".format(self.data_dir, file_name)
            if self.more:
                if os.path.exists(file_path) and attr not in self.loaded_attr:
                    logger.info("Loading data from {0} ...".format(file_path))
    ...
    def get_base_attribute_(self):
        ...
        logger.info(f"Num of speakers: {self.num_spks}")
    ...
    def generate(self, attr: str, spk2int: dict = None):
        ...
        if attr == "utt2spk_int":
            ...
            self.loaded_attr.append(attr)
        elif attr == "utt2domain_int":
            self.utt2domain_int = {}
            self.domain2int = {}
            for index, domain in enumerate(self.domain2utt):
                self.domain2int[domain] = index
            for utt, domain in self.utt2domain.items():
                self.utt2domain_int[utt] = self.domain2int[domain]
            self.loaded_attr.append(attr)
        else:
    ...
    def filter(self, id_list: set, id_type: str = "utt", exclude: bool = False):
        """
        id_list: a id set w.r.t utt-id or spk-id, dom-id. Could be list.
        id_type: utt, spk or domain.
        ...
        if id_type == "utt":
            ...
            for attr, file_name, value_type, vector in kaldi_dataset.domain_first_files:
                if attr in kaldi_dataset.loaded_attr:
                    if attr == "domain2utt":
                        this_file_dict = getattr(kaldi_dataset, attr)
                        if exclude:
                        ...
                        raise ValueError(
                            "Do not support file {0} w.r.t domain2utt only.".format(attr))
            ...
            if len(kaldi_dataset.utt_first_files) > 0:
                if "spk2utt" in kaldi_dataset.loaded_attr:
                    ...
                    for attr, file_name, value_type, vector in kaldi_dataset.domain_first_files:
                        if attr in kaldi_dataset.loaded_attr:
                            assert attr == "domain2utt"
                            this_file_dict = getattr(kaldi_dataset, attr)
                            if exclude:
                                new_file_dict = {
                                    k: sorted(list(set(v) - utt_id_list), key=lambda x: locale.strxfrm(x))
                                    for k, v in this_file_dict.items()
                                    if len(list(set(v) - utt_id_list)) != 0}
                            else:
                                new_file_dict = {
                                    k: sorted(list(set(v) & utt_id_list), key=lambda x: locale.strxfrm(x))
                                    for k, v in this_file_dict.items()
            ...
            raise ValueError(
                "Do not support id_type {0}, only utt, spk or domain.".format(id_type))

        kaldi_dataset.get_base_attribute_()

        return kaldi_dataset

    def subset(self, num_utts: int, requirement: str = "", drop=True, extra_list: list = []):
        """
    ...
    def split(self, num_utts: int, requirement: str = "", drop=True, extra_list: list = []):
        """
        @return: (KaldiDataset, KaldiDataset).
        """
        split_part = self.subset(num_utts, requirement, drop, extra_list)
...
class KaldiDatasetMultiTask():
    ...
    def subset(self, num_utts:int, requirement:str="", drop=True, extra_list:list=[]):
        ...
        if requirement == "--per-spk":
            ...
            # Select num_utts uttetances in total to contain speaker as more as possible. It is useful for val set.
            logger.info("Subset KaldiDatqaset to {0} utts with --total-spk requirement".format(num_utts))
            if len(self.utt2spk.keys()) < num_utts:
