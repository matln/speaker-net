...
from speakernet.utils.utils import read_dict, read_trials
...
def normalize(
    ...
    if not os.path.exists(mean_emb_path):
        logger.info("Not subtracting mean statistic when normalizing scores")
        mean_emb = 0.0
    else:
        if not os.path.exists(mean_emb_path):
    ...
    cohorts = list(read_dict(cohort_embs_scp).keys())
