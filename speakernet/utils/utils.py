    ...
    if use_gpu:
        ...
        if len(gpu_id) > 0 and use_ddp():
            ...
            model = torch.nn.parallel.DistributedDataParallel(
                model,
                device_ids=[gpu_id[dist.get_rank()]],
...
def write_nnet_config(model_blueprint: str, model_creation: str, nnet_config: str):
    ...
    logger.info(
        f"Save nnet_config to [bold cyan]{nnet_config}[/bold cyan] done.",
        extra={"markup": True},
    )
...
def create_model_dir(model_dir: str, debug: bool = False):
    if not debug and is_primary_process():
...
def set_seed(seed: int = 1024, deterministic: bool = True):
...
def load_data_csv(
    ...
    replacements : dict or str
        (Optional), e.g., {"/data/corpus": "/home/corpus"}, "/data/corpus:/home/corpus"
    ...
    with open(csv_path, newline="") as csvfile:
        ...
        if type(replacements) is str and ":" in replacements:
            replacements = {replacements.split(":")[0]: replacements.split(":")[1]}
        for row in reader:
            # ID:
            try:
...
def backup_code(args, hparams, model_dir, overrides, description="", log_env=True):
    if is_primary_process() and not args.debug:
        if "aug" in hparams["dataset_params"] and hparams["dataset_params"]["aug"]:
            ...
            shutil.copy(
                hparams["dataset_params"]["aug_conf"],
                f"{model_dir}/config/augmentation.yaml",
            )
            ...
            if exps != []:
                sorted_exps = sorted(
                    ...
                    key=lambda x: time.mktime(
                        time.strptime(x.replace("*", ""), "%Y-%m-%d_%H:%M:%S")
                    ),
                )
        ...
        # python environment
        if log_env:
            output_file = open(f"{model_dir}/config/environment.yaml", "w")

            proc1 = subprocess.Popen("conda env export", shell=True, stdout=subprocess.PIPE)
            proc2 = subprocess.Popen(
                "grep -v '^prefix: '", shell=True, stdin=proc1.stdout, stdout=output_file
            )
            proc1.stdout.close()
...
def input_with_timeout(prompt: str = "", timeout: int = 60):
    # set signal handler
    ...
    try:
        ...
        print("Time is up! Input is none")
        return ""
    finally:
...
def read_dict(file: str, value_type: str = "str", multi_value: bool = False, delimiter=" "):
    _dict = {}
...
# Define a shared timestamp class for sharing timestamps among different devices during multi-gpu training
class SharedTimer:
...
# Define remote functions for getting and updating the shared timestamp
def remote_get_shared_timestamp(timer):
    return timer.get_timestamp()


def remote_update_shared_timestamp(timer, new_timestamp):
    timer.set_timestamp(new_timestamp)


def get_remote_timestamp(timer):
    """
    Function to get remote timestamp using rpc_sync
    """
    # Get a reference to the remote timer
    worker = rpc.get_worker(f"timer:{dist.get_rank()}")
    return rpc.rpc_sync(worker, remote_get_shared_timestamp, args=(timer,))


def update_remote_timestamp(timer, new_timestamp):
    """
    Function to update remote timestamp using rpc_sync
    """
    # Get a reference to the remote timer
    worker = rpc.get_worker(f"timer:{dist.get_rank()}")
    remote_timestamp = rpc.rpc_sync(worker, remote_get_shared_timestamp, args=(timer,))
    if new_timestamp > remote_timestamp:
        rpc.rpc_sync(worker, remote_update_shared_timestamp, args=(timer, new_timestamp))
